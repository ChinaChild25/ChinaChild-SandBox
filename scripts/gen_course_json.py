#!/usr/bin/env python3
"""Generate data/courses JSON (no Node required). Source: scripts/gen-course-json.mjs logic."""
...
LESSON1_DATA["dialogueTexts"]["repeat"]["items"] = LESSON1_DATA["speakingPrompts"]
...
def write_json(rel_dir: str, name: str, obj: object) -> None:
    d = DATA / rel_dir
    d.mkdir(parents=True, exist_ok=True)
    (d / f"{name}.json").write_text(json.dumps(obj, ensure_ascii=False, indent=2), encoding="utf-8")


def stub_lesson(course_id: str, slug: str, title: str, topic_num: int) -> dict:
    return {
        "schemaVersion": 1,
        "slug": slug,
        "courseId": course_id,
        ...
        "source": {
            "note": (
                "Контент импортируется из презентаций курса «轻松学中文» и учебников Easy Step to Chinese (PDF) "
                "по пайплайну PPTX/PDF; не заполнять выдуманными фразами."
            ),
        ...
        "data": None,
        "catalogTitle": title,
        "catalogTopicNumber": topic_num,
    }


def stub_test(course_id: str, slug: str, title: str) -> dict:
    base = (
        "Playground/HSK1 Final Test Updated/ (block-01-core.html … — данные финального теста)"
        if course_id == "hsk1"
        else "HSK2: исходные блоки финального теста из базы HSK Lessons and Tests (импорт по аналогии с HSK1 Final Test Updated)"
    )
    return {
        "schemaVersion": 1,
        "slug": slug,
...
def main() -> None:
    ...
    write_json("hsk1", "hsk1-tema1", lesson1_file)

    for L in HSK1_LESSONS:
        if L["slug"] == "hsk1-tema1":
            continue
        write_json("hsk1", L["slug"], stub_lesson("hsk1", L["slug"], L["title"], L["n"]))
    for T in HSK1_TESTS:
        write_json("hsk1", T["slug"], stub_test("hsk1", T["slug"], T["title"]))

    for L in HSK2_LESSONS:
        s = stub_lesson("hsk2", L["slug"], L["title"], L["n"])
        s["source"]["markdown"] = ["Playground/hsk1_lesson_skeleton.md (структурный эталон для уроков)"]
        write_json("hsk2", L["slug"], s)
    for T in HSK2_TESTS:
        write_json("hsk2", T["slug"], stub_test("hsk2", T["slug"], T["title"]))
