...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => null)) as
    | { title?: string; description?: string; level?: string; levelCustom?: string; coverColor?: string; coverStyle?: string }
    | null
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      level: levelRaw || null,
      cover_color: body?.coverColor?.trim() || "var(--ds-sage-strong)",
      cover_style: body?.coverStyle?.trim() || "solid",
      teacher_id: me.id,
      is_custom: true,
