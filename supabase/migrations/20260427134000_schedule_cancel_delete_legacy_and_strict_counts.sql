-- Ensure cancel/reschedule clears legacy student slot time formats and avoid ghost lessons.
...
BEGIN
  ...
  v_time_legacy := CONCAT(EXTRACT(HOUR FROM (p_slot_at AT TIME ZONE v_timezone))::int, ':', to_char((p_slot_at AT TIME ZONE v_timezone), 'MI'));

  DELETE FROM public.student_schedule_slots
  WHERE student_id = p_student_id
    AND date_key = v_date_key
    AND (time = v_time OR time = v_time_legacy);
END;
$$;
